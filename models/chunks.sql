...
chunks as (
    select document.id document_id, splitter.id splitter_id, chunk_index, chunk, md5(document.id::text || splitter.id || chunk_index::text) id from document, splitter, pgml.chunk(splitter.name, document.text, splitter.parameters)
)
select * from chunks
